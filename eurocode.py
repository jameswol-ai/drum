...
def eurocode_uls_combinations(dead, live, wind=0, snow=0, seismic=0, psi0_live=0.7, psi0_wind=0.6, psi0_snow=0.5):
    """
    Generate ULS load combinations per EN 1990 Eq. 6.10.
    All inputs are characteristic load effects (e.g., bending moments, axial forces).
    Returns list of (name, value, gamma_G, gamma_Q).
    """
    combos = []
    
    # 6.10a - permanent + leading variable
    ...
    # 6.10b - permanent + leading variable + accompanying
...
def eurocode_sls_combinations(dead, live, wind=0, snow=0, psi1_live=0.5, psi1_wind=0.2, psi1_snow=0.2, psi2_live=0.3, psi2_wind=0.0, psi2_snow=0.0):
    """
    Generate SLS load combinations per EN 1990.
...
def en1992_rc_beam_design(b_mm, h_mm, d_mm, fck_mpa, fyk_mpa, M_ed_kNm, V_ed_kN, span_m, exposure_class="XC3", bar_dia=20):
    """
    Detailed RC beam design per EN 1992-1-1.
    ...
    # Concrete cover (simplified)
    cover = 25 + 10  # nominal cover + allowance
...
def en1993_steel_beam_design(section, fy_mpa, M_ed_kNm, V_ed_kN, span_m, buckling_check=True):
    """
    Steel beam design per EN 1993-1-1 with section classification.
    """
    sections = {
...
def en1995_timber_beam_design(timber_class, b_mm, h_mm, M_ed_kNm, V_ed_kN, span_m, service_class=2, load_duration_class="medium"):
    """
    Timber beam design per EN 1995-1-1.
    """
    timber_classes = {
    ...
    # Deflection (with creep for service class > 1)
...
def en1992_rc_column_design(N_ed_kN, M_ed_kNm, b_mm, h_mm, fck_mpa, fyk_mpa, l0_m):
    """
    RC column design per EN 1992-1-1 with second-order effects.
    """
    fck = fck_mpa
    fyk = fyk_mpa
    gamma_c = 1.5
    ...
    Ac = b_mm * h_mm
    As = 0.01 * Ac  # assume 1% reinforcement
    ...
    if lambda_ > lambda_limit:
        # Use moment magnification factor
